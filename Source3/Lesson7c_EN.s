...
; Lesson 7c.s    A SPRITE MOVED HORIZONTALLY USING A TABLE OF PRE-SET VALUES
;        (i.e. horizontal coordinates).
...
bsr.s    MoveSprite    ; Move sprite 0 horizontally
...
; in the TABX table. By acting only on HSTART, the scrolling is in 2
pixels at a time and not 1 pixel at a time, so it is slightly
jerky, especially during slowdowns. We will make the horizontal scrolling smoother
later, making it faithful to the single pixel.
...
MOVE.L    #TABX-1,TABXPOINT ; Start pointing to the first long again
NOBSTART:
...
Now replace it with this one, which makes the sprite oscillate on both sides.
