...
; Lesson 10e5.s    Deletion and drawing with Blitter Nasty
;        Left key to exit.

SECTION    CiriCop,CODE_C

; The code goes into CHIP memory to show the difference with the normal case
;
...
In this example, we show the effect of the Blitter Nasty bit.
As we explained, this bit only has an effect if the code resides
in chip memory and the processor's instruction cache is disabled.
To load the code into CHIP RAM, we specified a SECTION CODE_C
To disable the cache (if you have one), you can use the CPU
command of the operating system (or read the lesson on
680x0 processors!!). At this point (we are now basically in the same condition as
an Amiga 500), you can run the program by setting bit 10 of DMACON to 0 or 1
. To do this, simply change the DMASET label
at the beginning of the listing. If you set:

;5432109876543210
DMASET    EQU    %1000001111000000    ; copper,bitplane,blitter DMA
; Blitter Nasty OFF

You have Blitter Nasty (bit 10) at 0, and the processor occasionally has
priority.
If you set:
...
You have Blitter Nasty (bit 10) set to 1, and the blitter always has priority.
You can verify that in the second case, the blitting is faster.
