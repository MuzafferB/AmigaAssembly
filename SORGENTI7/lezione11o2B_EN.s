...
; Lesson 11o2B.s Loading a data file using dos.library
...
beq.s    NointVERTB        ; If yes, it is not a ‘true’ int VERTB!
...
move.w    d0,(a0)        ; Set the first colour saved as the last.

; Forward rotation of the second 15 colours

lea    cole-2,a0    ; Address of last colour in the second group
...
*****************************************************************************
; Routine that loads a file while we are typing in metal.
*****************************************************************************

DosLoad:
bsr.w    PrepareLoad    ; Restore multitasking and set load interrupt

moveq    #5,d1        ; number of frames to wait
bsr.w    WaitBlanks    ; wait 5 frames

bsr.s    LoadFile    ; Load the file with dos.library
move.l    d0,ErrorFlag    ; Save success or error status

bsr.w    mt_init		; Play the loaded music
...
; be able to access that file (you will not be able to delete or move it).
...
; Text string, to be terminated with a 0, which d1 must point to before
; opening dos.lib. It is best to put the entire path.

Filename:
dc.b    ‘assembler2:sorgenti4/mod.fairlight’,0    ; path+filename
even
...
myint6cLoad:
btst.b    #4,$dff01f    ; INTREQR - is bit 4, COPER, zeroed?
...
; Note that during loading, the interrupt is handled by the int ‘COPER’
*****************************************************************************
...
MOVE.W    $1E(A5),OLDINTREQL    ; Save the old INTREQ status
MOVE.L    #$80008000,d0		; Prepare the high bit mask
OR.L    d0,OLDDMAL        ; Set bit 15 of the saved
OR.W    d0,OLDINTREQL        ; register values so that they can be restored.
...
; (in kick 1.3 this is used to load from disk)
MOVE.L    4.w,A6
...
CMP.L    #$12d00,D0    ; wait for line $12d to avoid
...
beq.s    NoSpritez
bclr.l    #5,d0		; do not turn off sprites
NoSpritez:
MOVE.W    d0,$96(A5) ; DISABLE DMA

move.l    BaseVBR(PC),a0        ; In a0 the value of VBR
move.l	#MioInt6c,$6c(a0)    ; set my routine int. level 3.
...
*****************************************************************************
;         DESIGN 320*34 at 5 bitplanes (32 colours)
*****************************************************************************
...
******************************************************************************
; Buffer where the image is loaded from disk (or hard disk) via doslib
******************************************************************************
...
The key thing about loading is that the time you wait after loading,
before closing everything, otherwise it's game over!
In Interrupt, the colour routine is not executed, only the music routine
,
 at least you can see the time you wait “for safety”.
...
closing everything, at least you waited for the time, but not without using it!
