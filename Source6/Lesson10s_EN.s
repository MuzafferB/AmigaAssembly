...
; Lesson 10s.s    Correct filling of a closed polygon
...
; During the copy, the area is also filled. The type of fill
...
; the FILL_CARRYIN bit needs to be set
...
move.w    #12,$64(a5)        ; BLTAMOD width 14 words (40-28=12)
...
; point to the last word of the rectangle
; because of the descending mode

move.l    a0,$54(a5)		; BLTDPT loads the parameter
move.w    #(64*121)+14,$58(a5)    ; BLTSIZE (start the blitter!)
; width 14 words
; height 121 lines (1 plane)
rts


;******************************************************************************
; This routine draws a line using the special mode
; that allows the fill to be performed correctly. In addition, the line is
; drawn from top to bottom and the first bit is left unchanged.
; It takes as parameters the end points of the line P1 and P2, and the address of the
; bitplane on which to draw it.
...
DrawlineFill:
cmp.w    d1,d3        ; if D3>D1 the points are already in the right order
bge.w    Ordered
exg.l    d1,d3		; otherwise swap them
exg.l    d0,d2

Ordered:
sub.w    d1,d3        ; D3=DiffY (it is definitely positive)
...
lsr.w    #4,d1        ; clear the 4 low bits of X
add.w    d1,d1        ; obtain the X offset in bytes
add.w    d1,a0        ; adds the X offset to the address

and.w    #000F,d0    ; selects the 4 lowest bits of X..
move.w    d0,d1        ; .. copies them to D1..
...
move.l    a0,a1        ; copy the address
cmp.w    #7,d1        ; is the bit number > 7?
ble.s    NonCorreggi    ; if not, jump

addq.w    #1,a1        ; ..otherwise point to the next byte
subq.w    #8,d1		; and make the bit number < 7
NonCorrect:
not.b    d1        ; invert the bit numbering
; this instruction is necessary because
; within the byte, the bits are numbered 
; from right to left, while the coordinates
; of the pixels go from left to right
bchg    d1,(a1)		; invert the first pixel of the line

move.w    d2,d1        ; copy DX to D1
addq.w    #1,d1        ; D1=DX+1
lsl.w    #$06,d1        ; calculate in D1 the value to put in BLTSIZE
addq.w    #2,d1        ; add the width, equal to 2 words
...
or.w    #$0003,d5    ; set bit 0 (activate line mode) and
; bit 1 (special lines for fill)
...
In this example, we will see how to solve the problem of closed lines.
As explained in the lesson, we will modify the line drawing routine
so that it draws from top to bottom. To do this, at the beginning
, the Y coordinates of the two points are compared, and if necessary, the two points
are swapped. This also simplifies the calculation
of the octant, as we are limited to only four possibilities.
In addition, the first point of the line is inverted using a BCHG
to cancel the effect of the drawing. For this purpose,
a few more calculations are necessary to determine the bit number and the
correct address. Unlike the blitter, the BCHG operates one byte
at a time, so if necessary, you need to point to the low byte of the word
pointed to by the address. You also need to invert
 (with NOT) the bit numbering because the BCHG numbers the bits from right to left and
the coordinates are numbered from left to right.
The other routines are identical.
