...
; Lesson 10q.s    Filling a polygon with lines with a slope < 45 degrees
...
; During the copy, the screen is also filled. The type of fill
; is specified by the parameters.
; The parameters are:
; A0 - destination address
; D0 - if 0, perform inclusive fill, otherwise perform exclusive fill
; D1 - if 0, perform FILL_CARRYIN=0, otherwise perform FILL_CARRYIN=1
;****************************************************************************
...
move.w	#000a,d2        ; BLTCON1 value: set the bits for
...
; that allows the fill to be performed correctly. It takes as parameters the
; extremes of the line P1 and P2, and the address of the bitplane on which to draw it.
...
BITPLANE:
dcb.b    40*256,0        ; bitplane reset lowres
...
less than 45 degrees. Using the line drawing mode for filling
, we can fill the area correctly.
To draw the lines, we use the ‘DrawLineFill’ routine, which is identical
to the line drawing routine we have used so far, except that
it uses this special mode by setting the appropriate bit in BLTCON1.
As you can see before filling, this special mode
produces lines that always have only one pixel per horizontal line.
If the slope of the lines were > 45 degrees, the lines would be identical
to normal lines.
