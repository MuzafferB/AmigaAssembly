...
; Lesson 11o2B2.s Loading a data file using dos.library
...
move.w    #$c020,$9a(a5)    ; INTENA - enable ‘VERTB’ interrupt
; of level 3 ($6c)
...
*****************************************************************************
; Routine that loads a file while we are typing.
*****************************************************************************
...
bsr.s    CaricaFile    ; Load the file with dos.library
...
MOVE.L #filename,d1 ; Save its handle
MOVE.L #1,d2 ; Save its handle
MOVE.L #81158,d3 ; Save its handle
JSR -$42(A6)	; SEEK - ‘Seeka’ the file
...
MOVE.L    D0,D1        ; FileHandle in d1 for reading
...
MOVE.W    $2(A5),OLDDMAL        ; Save the old status of DMACON
...
dc.w    $b907,$fffe    ; WAIT - wait line $b9
BPLPOINTERS2:
dc.w $e0,0,$e2,0		;first bitplane
dc.w $e4,0,$e6,0        ;second
dc.w $e8,0,$ea,0        ;third	 ‘
...
from a diskette, you will notice that it appears plane by plane, in pieces, because it loads
