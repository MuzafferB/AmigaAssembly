...
; Lesson9i2r.s    BOB in colour (interleaved)
;        Left key to exit.
...
bsr.s	CheckCoordinates    ; prevent the bob from leaving the screen
...
; coordinate 319.
bls.s    controllaY	; If it is less, everything is fine, check Y
move.w    #319-32,ogg_x    ; otherwise set the coordinate on the edge.

controllaY:
tst.w    ogg_y        ; check top edge
bpl.s    NoMinY		; if it is positive, check at the bottom
...
or.w    #09f0,d0    ; ...correct to be inserted in the BLTCON0 register
...
move.w    #(64*11*3)+3,$58(a5)    ; BLTSIZE (start the blitter!)
rts
...
This example is the rawblit version of lesson9i2.s.
