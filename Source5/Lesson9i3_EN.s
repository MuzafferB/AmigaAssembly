...
; Lesson9i3.s    BOB with background restoration.
;        Left key to exit.
...
; BLTALWM = $0000 resets the last word
...
lea    4*11(a1),a1		; point to the next source plane
...
lea    40*256(a0),a0        ; point to next destination plane
lea    6*11(a1),a1        ; point to next source plane
; each blitter is 3 words wide and
; 11 lines high
dbra    d7,PlaneLoop3
rts
...
; the variables OGG_X and OGG_Y
;****************************************************************************

LeggiMouse:
move.b	$dff00a,d1    ; JOY0DAT mouse vertical position
...
; This is the buffer where we save the background each time.
...
In this example, we address the problem of the background with BOBs.
We will not yet provide the definitive solution, which requires an understanding of
blitter features not yet explained in the course. However, we will take
a first step. The idea is as follows: before drawing the BOB on the screen,
 we copy the part of the background that would be overwritten by the BOB into a buffer.
Then we draw the BOB as normal. After the vertical blank, we must delete
the BOB before redrawing it in its new position. Instead of simply deleting it
(which would leave the piece of screen empty), we copy
the background that was there before over it. This way, we delete the old copy
of the BOB and restore the background that was there before it passed.
The problem with this technique, as you can see, is that you cannot see the background
in the areas within the rectangle that encloses the BOB. This is because
the parts of the BOB coloured with colour 0 are not transparent
as they were for sprites, but represent the background colour.
