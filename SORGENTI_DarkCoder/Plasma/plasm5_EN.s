...
; Plasma5.s    2-bitplane RGB plasma and ripple
;        left button to exit
...
Largh_plasm    equ    36        ; plasma width expressed
; as number of groups of 8 pixels

; number of bytes occupied in the copperlist by each line of the plasma: each
; copper instruction occupies 4 bytes. Each line is formed by 1 ‘copper moves’ in
; BPLCON1, 1 WAIT,Largh_plasm ‘copper moves’ for the plasma

BytesPerRiga    equ    (Largh_plasm+2)*4

Alt_plasm    equ    235        ; plasma height expressed
; as number of lines

NuovaRigaR    equ    6        ; value added to the R index in the
; SinTab between one line and another
; Can be varied to obtain different plasmas
; but MUST ALWAYS BE EVEN!!

NewFrameR    equ    2        ; value subtracted from index R in
...
NewRowB    equ    -12        ; as “NewRowR” but for component B
NewFrameB    equ    -6        ; as ‘NewFrameR’ but for component B

NewRowO    equ    12        ; as ‘NewRowR’ but for oscillations
NewFrameO    equ    -4        ; as ‘NewFrameR’ but oscillations
...
MOVE.L    #PIC,d0        ; bitplane 2 (mask)
move.w    d0,14(a1)	; writes to copperlist 1 
move.w    d0,14(a2)    ; writes to copperlist 2
swap    d0
move.w    d0,10(a1)    ; writes to copperlist 1
move.w    d0,10(a2)    ; writes to copperlist 2
...
mod_D    set    BytesPerLine-2        ; channel D module: goes to next line
...
move.w    #-40,$108(a5)        ; BPL1MOD = -40 always repeat the same
; line
move.w    #0,$10a(a5)        ; BPL2MOD = 0 normal
move.w    #$2200,$100(a5)		; BPLCON0 - 2 bitplanes active
...
lea    19(a1),a1        ; address of the second byte of the second
...
move.b    d0,(a1)            ; write scroll value to
; ‘copper move’ in BPLCON1

lea    BytesPerRow(a1),a1    ; point to next row 
; in copper list
...
; its address to COP1LC. Swaps the variables so that in the next frame
...
move.l    d0,$80(a5)        ; copies the clist address
...
; WAIT instructions and the first half of the COPPERMOVE.
;****************************************************************************
...
; $24
move.w	#$184,d1    ; puts the first half of a 
; ‘copper move’ instruction in COLOR02 (=$dff184)
move.w    #$186,d4    ; puts the first half of a
; ‘copper move’ instruction in COLOR03 (=$dff186)
move.w    #$102,d5    ; puts the first half of an instruction in D4
...
; of which it writes a ‘column’ of plasma, i.e. it writes the colours in the
...
move.l    draw_clist(pc),a1    ; copperlist address where to write
lea    26(a1),a1        ; address of the first word of the first
; plasma column
...
moveq    #Largh_plasm-1,d2    ; the loop is NOT repeated for the entire
...
; calculate start address for component B
...
add.w    #NewRowG,d5        ; modify the index in the table
; for the next row
...
; modifies the B component index for the next line
...
; This table contains the oscillation values (scroll values of
; bitplane 1)
...
; Here, some space is left for the copperlist that generates
...
; it consists of 40 bytes alternating between 0 and $FF

BITPLANE:    dcb.w    20,$00FF

; Plasma mask: it is a 320*168 pixel image with 1 bitplane
...
In this example we see a plasma with 2 bitplanes. This is a
simple variant of the 1 bitplane plasma. Bitplane 1 is used to make the
plasma and 2 as a mask. The plasma uses colours 2 and 3.
The plasma will therefore appear at the pixels with a value of 1 in the
mask bitplane.
This technique also has the advantage that it is not necessary to write black
at the end of the line, because colour 0 (background) is not touched.
In this way, we can use a narrower plasma than the one
used in the example plasm4.s, even though the visible width is the same
(we save the last 2 ‘copper moves’ of each line; compare the different
values of the ‘Largh_plasm’ parameter).
Obviously, the mask bitplane is displayed normally, i.e.
without scrolling and with the module reset to zero.
