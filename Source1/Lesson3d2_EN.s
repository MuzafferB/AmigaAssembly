...
MoveCopper:
	LEA    BAR,a0
	TST.B	UpDown      ; Do we need to go up or down? If UpDown is
						; reset (i.e. TST checks BEQ)
						; then jump to GoDown, otherwise if it is at $FF
...
MettiGiu:
	clr.b    UpDown         ; By resetting UpDown, at TST.B UpDown the BEQ
	rts           			; will jump to the GoDown routine, and
							; the bar will drop

GoDown:
	cmpi.b    #$fa,8*9(a0)  ; have we reached line $fc?
	beq.s	PutUp        	; if so, we are at the bottom and we have to go back up
							;    addq.b    #1,(a0)
...
In this example, the MoveCopper routine is executed once every 3 FRAMES,
