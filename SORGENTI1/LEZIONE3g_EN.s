...
; where the number of times the VAIDESTRA or VAISINISTRA routine has been
...
VAIDESTRA is executed, the ‘grey bar’ moves to the right, so we have to
...
by executing the VAISINISTRA routine 85 more times, which returns it to
its initial position, and the cycle restarts to continue until we press
the mouse button.
; PLEASE NOTE THAT THIS ROUTINE EITHER GOES TO VAIDESTRA OR VAISINISTRA, NOT
; BOTH: IF VAIDESTRA IS EXECUTED, THEN IT RETURNS TO THAT
; ROUTINE TO THE MOUSE LOOP:, THE SAME FOR VAISINISTRA. IF THE VAIDESTRA AND
; VAISINISTRA IS FINISHED (AFTER 2*85 FRAMES), RETURN TO THE ‘MOUSE’ CYCLE FROM THE RTS
; OF THE CopperDESTSIN ROUTINE directly, after resetting the 2 flags.


CopperDESTSIN:
CMPI.W    #85,RightFlag        ; VAIDESTRA executed 85 times?
BNE.S    VAIDESTRA        ; if not yet, re-execute it
; if it has already been executed 85
; times, continue below

CMPI.W    #85,LeftFlag    ; VAISINISTRA executed 85 times?
BNE.S    LEFT        ; if not yet, re-execute it
...
; VAIDESTRA will be executed again, after 85 frames
; vaisinistra 85 times for 85 frames, etc.
...
addq.w    #1,DestraFlag    ; mark that we have executed another time
; VAIDESTRA: DestraFlag contains the number
; of times we have executed VAIDESTRA.
RTS            ; RETURN TO THE LOOP mouse


VAISINISTRA:            ; this routine moves the bar to the LEFT
subq.b    #2,CopBar    ; subtract 2 from the X coordinate of the wait
addq.w    #1,SinistraFlag ; Add 1 to the number of times
; VAISINISTRA has been executed.
RTS            ; RETURN TO THE LOOP mouse
...
SinistraFlag:        ; This word keeps track of the number of times
...
BNE.S    VAIDESTRA
CMPI.W    #85/2,SinistraFlag    ; 85/2, i.e. 42 times
BNE.S    VAISINISTRA        ; if not yet, run it again
....
...
MoveCopper:
cmpi.b    #$e1,BARRA+1    ; have we reached column $fc?
beq.s    Finished        ; if so, we are at the bottom and do not continue
addq.b    #2,BARRA+1    ; WAIT 1 changed, the bar advances by 2
Finished:
rts
...
for generating effects with the copper, and therefore for understanding the copper itself. In
Lesson 4, however, the 68000 routines will be even simpler than those
