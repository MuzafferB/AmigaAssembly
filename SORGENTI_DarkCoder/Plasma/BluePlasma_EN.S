...
; Plasma 2 bitplanes - Original author: unknown
;
; Modified/Fixed by Randy/Ram Jam in 1995
...
Start:
bsr.w    make_stable    ; makes mempty stable

bsr.w    setup_copcols    ; creates the copperlist
...
bsr.s    fadeeffect    ; Initialise everything (as if doing 1 loop)
bsr.w    sine_vert    ; Vertical sine with bleed loop
bsr.w    sine_horiz    ; Modify bplcon1 in copperlist with the CPU

move.l    #NewCopper,$80(a5)    ; Point to our COP
...
bsr.s    fadeeffect    ; Choose effect
bsr.w    sine_vert    ; Vertical sine with bleed loop
bsr.w    sine_horiz    ; Modify bplcon1 in copperlist with CPU
...
;*****************************************************************************
;    Change effects according to the list.
;*****************************************************************************
...
setup_copcols:
lea    copcols,a0    ; colours in coplist
move.w    #$4039,d2    ; wait
move.w    #(depth/2)-1,d7
sc_loop1:
move.w    #(width/2)-1,d6
...
move.w    d7,(a0)+
move.w    d2,(a0)        ; Wait first word
addq.w    #1,a0		; second byte (pos. XX)
bset.b    #0,(a0)+
add.w    #$0100,d2    ; Wait 1 line below
move.w    #$fffe,(a0)+    ; wait second word
sc_loop2:
...
;*****************************************************************************
; Modify bplcon1 ($dff102) in copperlist for horizontal scrolling
;*****************************************************************************
...
make_stable:
move.l    mempty(PC),a0
move.w    #$ffff/4,d7    ; 16384 words, i.e. 32768 bytes
moveq    #0,d6
ms_more:
move.w    d6,d0
addq.w    #4,d6
