...
move.l    #NewIRQ,$6c(a0)     ; I put my level 3 int. rout.

MOVE.L    #plane1,D0     ; planes
LEA    BplPointers,A1     ; pointer address in the copperlist.
...
; and sprites.
move.l    #Newclist,$80(a5)    ; Point to our COP
...
; 5432109876543210
move.w    #%1100000000100000,$9a(a5) ; INTENA - enable only VERTB
...
lll:    btst    #6,$bfe001    ; swappes page with left key
bne.s    lll
...
LEA    BplPointers,A0     ; pointer addresses in the copperlist.
move.w    d0,6(a0)
swap    d0
move.w    d0,2(a0)
...
; only if DL_Fill=1
move.b    d4,d5        ; calculate number of bits to invert
not.b    d5        ; (BCHG numbers bits in reverse order)    
ENDC
...
;ннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннннн
...
beq.w    NOIRQ_6c    ; If yes, it is not a ‘real’ int VERTB!
movem.l    d0-d7/a0-a6,-(sp)

; Now let's handle the generator point that flickers on the screen.
...
move.w    d0,OldGunX
move.w    d1,OldGunY
move.l	SavedSCR(PC),a0    ; destination plane
...
mulu.w    #40,d1        ; y offset (Y* screen width)
...
bclr.b    d2,(a0,d0.w)    ; Reset bit d2 to the correct offset

; Now let's establish the new position: read the values of ADD

lea    SinPX0(PC),a0    ;Harmonic Motion {
move.w    (a0),d0        ; x0
move.w    2(a0),d2    ; x1
move.w    4(a0),d1    ; y0
move.w    6(a0),d3    ; y1
...
; Now let's take the values from the synthab and save them for the next steps
...
asr.w    #4,d0	; /16
asr.w    #4,d1    ; /16
add.w    #160,d0    ; centreX
add.w    #128,d1    ; centreY
move.w    d0,GunX
move.w    d1,GunY

; plot the new point

move.l    CurScreen(PC),a0    ; destination plane
move.l    a0,SavedSCR
...
and.w    #%111,d2    ; Select only the first 3 bits of X, i.e.
not.w    d2        ; appropriately noted
bset.b    d2,(a0,d0.w)    ; Set bit d2 to the correct offset
...
ChPer:
dc.w    0.3,6,3, 3.0,3.6, 1.1,5.5, 1.5.5, -1
ChPiu:
...
ChE:
dc.w	0.0,0.0,6, 0.0,6,0, 0.3,5,3, 0.6,6,6, -1
ChF:
dc.w    0.0,0.0,6, 0.0,6,0, 0.3,5,3, -1
ChG:
...
ChJ:
dc.w    0.5,1.6, 1.6,5.6, 5.6,6.5, 6.5,6.0, -1
ChK:
...
dc.b    11,0,40,2,‘1 - INFINITE DREAMS’,10
...
dc.w    $0180,$0000    ; colour0 - black (background)
dc.w    $0182,$0ff0    ; colour1 - yellow (cursor and gunwriter)
dc.w    $0184,$0fff    ; colour2 - white (stopped letters)
dc.w    $0186,$0ff0	; colour3 - yellow (gunwriter+letters)
