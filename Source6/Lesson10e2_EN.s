...
; Lesson 10e2.s    Deletion and drawing with copper monitor
;        Left key to exit.
...
In this example, we move a rectangle on the screen, i.e. we delete it
and redraw it. We have used a series of different colours to
highlight the speed of the routines and the work done by the blitter and the
processor. When the erase routine starts, the screen turns
light red. At this point, both the blitter and the processor
are busy with the erase operation. When the processor finishes
the erase routine, the blitter has not yet finished executing
its task. However, the processor is already free for other tasks. This
situation is highlighted by colouring the screen a darker red
at the end of the ‘ClearObject’ routine. At this point, the processor
enters the ‘DrawObject’ routine. Since the blitter is still busy
before the drawing operation begins, it is necessary to wait for the
blitter. When the blitter finally becomes free, the screen is coloured
light green. At the exit of the ‘DrawObject’ routine, the screen is
coloured dark green while waiting for the blitter to finish drawing.
When this happens, the screen turns black again.

Also note that erasing and copying require (approximately) the same
speed.
