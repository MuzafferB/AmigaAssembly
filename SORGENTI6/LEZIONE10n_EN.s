...
btst    #6,2(a5)
WaitLine:
btst    #6,2(a5)    ; wait for blitter to stop
bne    WaitLine
...
In this example, we present line drawing. This is achieved
using three different routines.
The ‘InitLine’ routine sets the registers whose content is independent of the
line parameters (the end points) and which can therefore be set
once at the beginning of the programme.
The ‘SetPattern’ routine defines the pattern to be used for a line.
Every time you want to use a different pattern, you must execute this 
routine. Conversely, if there are several lines to be drawn with the same
pattern, this routine can be executed only once.
The ‘Drawline’ routine is the routine that actually draws the line and
is also the most complex. At the beginning, DX and DY are calculated based on the
coordinates of the end points, and the octant code to be used is determined.
To perform these operations, a series of subtractions and
comparisons are necessary, which examine all possible cases.
Next, the values to be entered in the other registers are calculated,
as explained in the comments.
Note that LF=$4A is used, which causes an EOR between the line and the background.
You can see this by looking at the two intersecting lines: the intersection is
a pixel with a value of 0. If you try setting LF=$CA, you will see that the intersection
is instead a pixel with a value of 1.
