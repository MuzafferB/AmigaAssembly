...
; Lesson 10e3.s    3-channel operation with copper monitor
;        Left key to exit.
...
or.w    #$0dfc,d0    ; ...just right to fit into the BLTCON0 register
lsr.w    #3,d1        ; (equivalent to a division by 8)
; round to multiples of 8 for the pointer
; to the screen, i.e. to odd addresses
...
move.l    #$00000004,$64(a5)    ; BLTAMOD=$fffe=-2 go back
...
OGG_Y:        dc.w    0	; here the Y of the object is stored
OGG_X:        dc.w    0    ; here the X of the object is stored
MOUSE_Y:    dc.b    0    ; here the Y of the mouse is stored
MOUSE_X:    dc.b    0    ; here the X of the mouse is stored
...
; this is the dummy plane for the OR. It has the same dimensions as the bob
; and is completely cleared.
...
In this example, instead of making a copy from A to D, we do an OR between
channels A and B and write the result in D. In practice, we always get
the same thing, because channel B reads a plane made up of all 0s,
but this example serves to show how a 3-channel bleed is
slower than a 2-channel bleed.
