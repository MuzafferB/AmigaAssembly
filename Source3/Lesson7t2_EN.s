...
; Lesson 7t2.s    Slow stars

;    In this listing, we will create a starry sky
;    with stars that move one pixel at a time
...
; acting on each HSTART (and on the VHBITS byte for smooth scrolling of 1 pixel
; instead of 2) through the STELLE_LOOP loop
...
BNE.S    no_edge        ; no, then jump
MOVE.B    #$30,1(A0)        ; yes, put the star back on the left
no_bordo:
BCHG    #0,3(A0)        ; act on the VHBITS byte
BEQ.S    even
...
; The vertical positions go in pairs, so there is one star every two
; vertical lines, with a different horizontal position for each star.
...
In this listing, we move the stars one pixel at a time.
The program is identical to the previous example, the only thing that changes is
that we have to increase the horizontal positions of the stars by 1 pixel instead of 2
when we move them. The problem is that, as you know, the low bit
of the horizontal position of a sprite is written separately from the
others, so we cannot move a sprite by 1 pixel with a simple
ADD.
 One possible solution would be to store the horizontal positions separately in word form, i.e. with all the bits together,
add 1 to the words and then write them to the HSTART byte and the low bit
using the same procedure we used in the UniMuoviSprite routine.
However, this solution would force us to store 127 words, one for
each horizontal position. 
In the routine, we used a better method.
...
this instruction works exactly like BSET or BCLR, except that
while BSET always sets the indicated bit to 1 and BCLR always sets it to 0,
BCHG inverts it: if it was 0 before, it turns it into 1 and vice versa. In this case
the bit in question is the low bit of the star's position.
Furthermore, BCHG BEFORE INVERTING THE BIT, checks if it is 0, exactly
as BTST does. Therefore, the instruction

BEQ.S    even

performs the jump if the low bit of HSTART WAS 0 BEFORE EXECUTING THE BCHG.

The following instruction moves the sprite by 2 pixels if the position was odd

ADDQ.B #$1,1(A0) ; moves the sprite by 2 pixels.

These 3 instructions solve our problem. In fact, there are
two possible cases:

If the low bit WAS 0, BCHG sets it to 1, which is equivalent to adding
1 to the horizontal position. Since in this case BEQ performs the
jump, the HSTART byte is not modified. In practice, if, for example, we had
byte HSTART=%0110010 and low bit = 0 (which together formed the
horizontal position %01100100=100), after executing the routine
we would find HSTART equal and the low bit=1, i.e.
HSTART byte = %0110010 and low bit = 1 (which together form the
horizontal position number %01100101=101).
As desired, we have increased the horizontal position by 1.

If, on the other hand, the low bit WAS 1, the BCHG sets it to 0, which is equivalent to
subtracting 1 from the horizontal position. In this case, however, the BEQ
does NOT perform the jump, and therefore 1 is added to the HSTART byte, which,
 as you know, is equivalent to adding 2 to the horizontal position. By combining
the subtraction of 1 performed by the BCHG with the addition of 2 performed by the
ADD, we have added 1 to the horizontal position.

In practice, if, for example, before executing the routine we had
byte HSTART=%0110010 and low bit = 1 (which together formed the
horizontal position number %01100101=101), after executing the routine
we find ourselves with HSTART increased by 1 and the low bit=0, i.e.
byte HSTART=%0110011 and low bit = 0, which together form the
horizontal position %01100110=102).
As we wanted, in this case we have also increased the horizontal position of the sprite by 1.
The same happens with the other sprites.
