...
; Lesson 10f2.s    Many BOBs with ‘fake’ background and ‘double buffering’
;        Left key to exit.
...
START:
; the first 3 planes are pointed by the SwapBuffer routine

;    We point the fourth bitplane (the background)
...
;****************************************************************************
; This routine swaps the two buffers by swapping the addresses in the
; variables VIEW_BUFFER and DRAW_BUFFER.
; It also updates the instructions in the copperlist that load the registers
; BPLxPT, so that they point to the new buffer to be displayed.
;****************************************************************************
...
neg.w    d3            ; change the sign of the speed dy
...
; be inserted into the shifter of channel A
...
view_buffer    dc.l    BITPLANE1    ; displayed buffer
...
; the background is displayed in this space
...
; These are the bitplanes of the first buffer
BITPLANE1:
ds.b    40*256
BITPLANE2:
ds.b    40*256
...
In this example, we solve the problem in lesson10f1.s using
the double buffering technique. This technique consists of using two
separate buffers, one that is displayed and one that is drawn on
which are exchanged at each Vertical Blank.
The addresses of the two buffers are contained in two variables. The variable
draw_buffer contains the address of the buffer used to draw in the
current frame. The routines that modify the screen (ClearScreen
and DrawObject) read the address of the drawing buffer from draw_buffer
and perform their operations on it.
The view_buffer variable contains the address of the buffer currently
displayed.
The SwapBuffer routine swaps the contents of the two variables at each frame
. It also ensures that the correct
buffer (the one whose address is stored in view_buffer) is displayed. In fact,
to display the two buffers alternately, it is necessary that at each
frame the copper list points the BPLxPT registers to the bitplanes of the
display buffer. This means that the copper list must be
modified at each vertical blank, which is done by the
SwapBuffer routine.
