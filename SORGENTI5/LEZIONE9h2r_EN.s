...
; Lesson 9h2r.s    BLITTATA, in which we copy a rectangle (in a RAWBLIT pic)
;        not aligned with a word, using masks to ‘fill in’.
;        Pressing the right key performs the ‘dirty’ blitting
;        Then, pressing the left key performs the correct blitting
...
lea    bitplane+((20*3*170)+80/16)*2,a0    ; destination ind.
...
; Second blitting, thanks to the masks only the letter ‘I’ is copied
;

lea    bitplane+((20*3*170)+160/16)*2,a0    ; destination ind.
...
; load the parameters into the masks
move.w    d0,$44(a5)        ; BLTAFWM mask on the left
move.w    d1,$46(a5)		; BLTALWM mask to the right

; load pointers

move.l    #bitplane+((20*3*78)+128/16)*2,$50(a5) ; bltapt: fixed source
move.l    a0,$54(a5)             ; bltdpt: destination

move.l #$00240024,$64(a5)        ; bltamod and bltdmod 

move.w    #(3*60*64)+2,$58(a5)        ; bltsize
; height 60 lines and 3 planes
; width 2 words
...
This example is the rawblit version of lesson9h2.s.
