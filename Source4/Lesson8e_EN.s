...
; A FADE routine (i.e. fading) with a modification concerning the colour mix
; in fact, you can indicate an RGB shade to
; aim for. Press the left and right keys several times to view and exit.
...
;    ****************+ part 1 with RED tint *****************

clr.w    FadePhase    ; reset the frame number
move.w    #$b12,Tendency    ; set the tendency to red ******
...
;    ****************+ part 2 with tendency to GREEN *****************

clr.w    FadePhase    ; reset the frame number
move.w    #$373,Tendency    ; set the tendency to green ******
...
;    ****************+ part 3 with BLUE trend *****************

clr.w    FadePhase    ; reset frame number
move.w    #$33c,Tendency    ; set BLUE tendency ******
...
*****************************************************************************
;    Routines that wait and call Fade at the right moment
*****************************************************************************
...
FadeOut:
tst.w    FaseDelFade    ; have we passed the last phase? (16)?
beq.s    FinitoOut
...
FadePhase:        ; current phase of the fade (0-16)
dc.w    0

*****************************************************************************
*        Routine for Fade In/Out from and to WHITE         *
* Input:								 *
*
...
* d6 = colour to add to the fade to give it certain shades *
*
...
*									 *
* The modification simply consists of adding the tones in d6 *
* and dividing the result by 2, to avoid exceeding a total of $f *
*
*****************************************************************************
...
Fade:
MOVE.W	Trend(PC),D6    ;    ; RGB trend mask
Fade1:
MOVE.W    D6,D1        ; copy trend col. to d1
MOVE.W    D6,D2        ; to d2
MOVE.W    D6,D3        ; to d3
ANDI.W    #$00f,D1	; SELECT BLUE ONLY
ANDI.W    #$0f0,D2    ; SELECT GREEN ONLY
ANDI.W    #$f00,D3    ; SELECT RED ONLY
...
; modify
ADD.W    D1,D4        ; Add the BLUE component of Trend
LSR.W    #1,D4        ; and divide by 2 using a 1-bit shift to the right
; end of modification
...
; modify
ADD.W    D2,D4        ; Add the GREEN component of Tendency
LSR.W    #1,D4        ; and divide by 2 using a 1-bit shift to the right
; end of modification
MULU.W    D0,D4        ; Multiply by the fade phase (0-16)
ASR.W    #4,D4        ; shift 4 BITS to the right, i.e. division by 16
...
; modify
ADD.W    D3,D4        ; Add the RED component of Trend
LSR.W    #1,D4		; and divide by 2 using a 1-bit shift a >
; end of modification
MULU.W    D0,D4        ; Multiply by the fade phase (0-16)
ASR.W    #4,D4        ; shift 4 BITS to the right, i.e. division by 16
AND.W    #$f00,D4	; Select only the red component ($RGB->$R00)
OR.W    D4,D5        ; Save the RED colour together with BLUE and GREEN

MOVE.W    D5,(A1)        ; And put the final $0RGB colour in the copperlist
addq.w    #4,a1        ; next colour in the copperlist
DBRA	D7,Fade1    ; do all colours
rts
...
; 5432109876543210
dc.w    $100,%0100001000000000    ; BPLCON0 - 4 lowres planes (16 colours)
...
This is a simple modification of the previous routine.
