...
; Lesson 10b1.s    Example of OR between 2 channels
...
bsr.s    BlitOR		; execute OR between the 2 figures
...
;****************************************************************************
; This routine performs an OR between 2 figures read through channels A and B
;****************************************************************************
...
move.l    #$ffffffff,$44(a5)    ; masks
move.l    #$0dfc0000,$40(a5)    ; BLTCON0 and BLTCON1
; use channels A, B and D
; perform OR between A and B (LF=$FC)
move.w    #0,$64(a5)        ; BLTAMOD (=0)
move.w    #0,$62(a5)        ; BLTBMOD (=0)
move.w    #30,$66(a5)        ; BLTDMOD (40-10=30)

move.l    #Figure1,$50(a5)		; BLTBPT source pointer
move.l    #Figure2,$4c(a5)        ; BLTAPT source pointer
...
In this example, we see the OR between two figures. This is a simple
blit that performs the OR between channels A and B, using the LF value
calculated in the lesson. For practice, you can modify it using
to read channel C instead of B. The changes to be made are
as follows:
Replace the module and pointer registers of channel B with those of C;
activate channel C instead of B;
Calculate the correct LF value to perform the OR between A and C.
The LF calculation is simple: just look at the table in fig. 27 and
set all minterms corresponding to combinations with A=1 or
with C=1 to 1. This gives LF=$FA.
Repeat the same exercise to perform the OR between channels B and C.
